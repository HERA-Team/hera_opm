...
# 9 - fraction of flagged visibilities, above which antenna gain is flagged
fn="${1}"
ft="${2}"
model_files="${3}"
phs_max_iter="${4}"
...
omni_abscal_run.py --refant ${refant} --max_bl_cut ${max_bl_cut} --edge_cut ${edge_cut} --antflag_thresh ${antflag_thresh} --avg_dly_slope_cal --delay_slope_cal --phase_slope_cal --TT_phs_cal --abs_amp_cal --phs_max_iter ${phs_max_iter} --phs_conv_crit ${phs_conv_crit} --overwrite --input_cal ${omni_fn} --output_calfits_fname ${calfits_fn} --data_file ${fn} --filetype ${ft} --model_files ${model_files} 
